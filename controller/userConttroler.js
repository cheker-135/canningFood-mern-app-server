...
exports.loginUser = asyncWrapper(async (req, res, next) => {
  ...
  if (!email || !password) {
    return next(new ErrorHandler("Entrer votre email et mot de passe s'il vous plat!!", 400));
  }
  ...
  if (!user) {
    return next(new ErrorHandler("Email ou mot de passe invlaides", 401));
  }
...
exports.logoutUser = asyncWrapper(async (req, res) => {
  ...
  res.status(200).json({
    success: true,
    message: "Admin Deconnecté avec succes",
  });
});
...
exports.forgotPassword = asyncWrapper(async (req, res, next) => {
  ...
  try {
    await sendEmail({
      ...
      email: user.email,
      subject: `Conserverie menzah 9 Password Recovery`,
      message,
    });
